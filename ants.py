...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        placment = self.place
        while placment != hive:
            if random_or_none(placment.bees) and (self.min_range <=counter < self.max_range):
                return random_or_none(placment.bees)
            placment = placment.entrance
            counter += 1
        return None
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        listbe= self.place.bees
        list2 = listbe[:]
        for i in list2:
            Insect.reduce_armor(i, self.damage)


# first modify the Ant class to include a new class attribute blocks_path that is True by default. 
# Set the value of blocks_path to False in the NinjaAnt class.
# Second, modify the Bee's method blocked to return False if either there is no Ant in the Bee's place or if there is an Ant, 
# but its blocks_path attribute is False. Now Bees will just fly past NinjaAnts.
# Finally, we want to make the NinjaAnt damage all Bees that fly past. 
# Implement the action method in NinjaAnt to reduce the armor of all Bees in the same place as the NinjaAnt by its damage attribute. 
# Similar to the FireAnt, you must iterate over a list of bees that may change.
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, 1)
        if QueenAnt.numberqueen >= 1:
            self.truequeen = True
        else:
             self.truequeen = False
        QueenAnt.numberqueen =+ 1
    ...
    def action(self, colony):
        ...
        if self.truequeen == True:
            ...
            while position is not None:
                antposition = position.ant
                if antposition == True and antposition not in QueenAnt.ants:
                    antposition.damage *= 2
                    self.aldoubled.append(antposition)
                if antposition.is_container == True:
                    if antposition.contained_ant == True and antposition.contained_ant not in self.ant:
                        antposition.contained_ant.damage *=2
                        self.aldoubled.append(antposition.contained_ant)
                antposition = position.exit
